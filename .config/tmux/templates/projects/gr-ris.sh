...
tmux split-window -v -t "${name}:run.3" -c "$path"
...
tmux send-keys -t "${name}:run.1" '' C-m
tmux send-keys -t "${name}:run.2" 'pnpm --filter web run dev' C-m
tmux send-keys -t "${name}:run.3" 'pnpm --filter admin-web run dev' C-m
tmux send-keys -t "${name}:run.4" 'pnpm run build:watch' C-m
