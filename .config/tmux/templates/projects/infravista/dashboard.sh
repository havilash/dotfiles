...
tmux split-window -v -t "${name}:run" -c "$path"
...
tmux send-keys -t "${name}:run.1" 'cd app; npm run dev' C-m
tmux send-keys -t "${name}:run.2" 'cd server; npm run dev' C-m
tmux send-keys -t "${name}:run.3" 'cd infravista-common; npm run build:watch' C-m
